...
@if (session()->has('success'))
    <div class="alert alert-success alert-dismissible fade show" role="alert" id="message_alert">
        ...
        <div class="alert-message">
            <h4 class="alert-heading">{{ session('response')->status }}</h4>
            <p>{{ session('success') }}</p>
        </div>
    </div>
@endif
        ...
        <form method="GET">
            <select class="form-select form-control" name="page_size" onchange='this.form.submit()'>
                ...
                @foreach ($page_size_options as $option)
                    <option value="{{ $option }}" @if (request('page_size') == $option) selected @endif>{{ $option }}</option>
                @endforeach
            </select>
        </form>
    </div>
...
<table class="table table-sm table-striped align-middle">
    ...
    <tr>
        ...
        <td>
            ...
            data-id="{{ $data->id }}" data-name="{{ $data->name }}" data-price="{{ $data->price }}">
                Edit
            </button>
...
{{-- Modal --}}
{{-- Modal Create --}}
<div class="modal fade" id="modal_create" data-bs-backdrop="static" data-bs-keyboard="false"
            ...
            <div class="modal-body">
                <form method="POST" id="form_create">
                    ...
                    <div class="form-floating mb-3">
                        <input type="text" name="name" class="form-control @if (isset(session('response')->errors->name[0])) is-invalid @endif" placeholder="Name" required
                            value="{{ old('name') }}">
                        <label>Name</label>
                    ...
                    <div class="form-floating mb-3">
                        <input type="number" name="price" min="1" class="form-control @if (isset(session('response')->errors->price[0])) is-invalid @endif" placeholder="Price" required
                            value="{{ old('price') }}">
                        <label>Price</label>
